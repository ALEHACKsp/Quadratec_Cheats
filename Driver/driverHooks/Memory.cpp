...
/*Gets a base address of whatever lol*/



PVOID functions::DriverHooks::get_system_module_base(const char* module_name) {
...
/*its self explantory lmao*/
PVOID functions::DriverHooks::get_system_module_export(const char* module_name,LPCSTR routine_name)
{
	PVOID lpModule = functions::DriverHooks::get_system_module_base(module_name);
...
bool functions::DriverHooks::ReadMemory(void* address, void* buffer, size_t size) {
...
bool functions::DriverHooks::WriteMemory(void* address, void* buffer, size_t size) {
...
bool functions::DriverHooks::WriteToReadOnlyMemory(void* address, void* buffer, size_t size) {
...
bool functions::DriverHooks::CallKernelFunction(void* kernel_function_address) {
	if (!kernel_function_address)
		return false;

	PVOID* dxgk_routine
		= reinterpret_cast<PVOID*>(functions::DriverHooks::get_system_module_export("\\SystemRoot\\System32\\drivers\\dxgkrnl.sys", "NtOpenCompositionSurfaceSectionInfo"));
	...
	*/
	...
	using namespace functions::DriverHooks;
...
bool functions::DriverHooks::WriteKernelMemory(HANDLE pid, uintptr_t address, void* buffer, SIZE_T size) {
...
NTSTATUS functions::DriverHooks::protect_virtual_memory( ULONG64 pid, PVOID address, ULONG size, ULONG protection, ULONG& protection_out )
{
...
bool functions::DriverHooks::ReadKernelMemory(HANDLE pid, uintptr_t address, void* buffer, SIZE_T size) {
	if (!address || !buffer || !size)
		return false;
	SIZE_T bytes = 0;
	NTSTATUS status = STATUS_SUCCESS;
...
ULONG64 functions::DriverHooks::GetModuleBasex64(PEPROCESS proc, UNICODE_STRING module_name) {
